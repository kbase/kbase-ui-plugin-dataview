    ...
    */
    ...
    class DataTable6 extends Component {
        constructor(props) {
            ...
            this.columnMap = this.props.columns.reduce((columnMap, column) => {
            ...
            const [table, sort] = (() => {
                if (this.props.initialSort) {
                    const sort = [{
                        column: this.props.initialSort.column || 0,
                        direction: this.props.initialSor.direction || 'ascending'
                    }];

                    return [this.sortTable(ordered, sort), sort];
                }
                return [ordered, null];
            })();
        ...
        isColumnSorted(sortable, columnIndex) {
            if (!sortable || this.state.sort === null) {
                return false;
            }
        ...
        renderSortableIndicator(sortable, columnIndex) {
            ...
            return html`
                <div className="DataTable5-header-col-sortable" title=${title}>
                    ${this.renderSortPosition(sortIndex)} <span className=${['fa', 'DataTable5-sort-icon', sortIcon].join(' ')} style=${sortStyle}/>
                </div>
            `;
        }
        ...
        getRowValue({rowIndex}, columnIndex, valueKey) {
            const columnId = this.props.columns[columnIndex].id;
            const value = this.tableMap[rowIndex].values[columnId];
            if (valueKey) {
        ...
        sortTable(table, sort) {
            const compare = (a, b, column) => {
                const columnSpec = this.props.columns[column];
                const aRow = this.tableMap[a.rowIndex];
                const bRow = this.tableMap[b.rowIndex];
                const aValue = this.getRowValue(a, column, columnSpec.sortKey);
                const bValue = this.getRowValue(b, column, columnSpec.sortKey);
                const customComparator = columnSpec.sortComparator;
                if (customComparator) {
                    return customComparator(aValue, bValue, aRow, bRow);
                }
        ...
        addNewSort(sortColumn) {
            ...
            const sort = this.state.sort === null ? [] : this.state.sort.slice();
            sort.push(newSort);
                ...
                return (() => {
                    const header = this.props.columns.map(({sortable, label, styles}, columnIndex) => {
                        const headerColClasses = [
                            'DataTable5-header-col'
                        ];
                        if (sortable) {
                            headerColClasses.push('DataTable5-header-col-sortable');
                        }
                        if (this.isColumnSorted(sortable, columnIndex)) {
                            headerColClasses.push('DataTable5-header-col-sorted');
                        }
                        const isHovered = this.state.columnHovered === columnIndex;
                        if (isHovered) {
                            headerColClasses.push('DataTable5-col-hovered');
                        }
                        return html`
                            <div className=${headerColClasses.join(' ')} 
                                ...
                                <div className="DataTable5-header-col-label" >
                                    ${label}
                                </div>
                    ...
                    header.unshift(html`
                        <div className="DataTable5-header-col" style=${{flex: '0 0 2em'}}>
                            <${IconButton}
                                icon="filter"
                                activeIcon="close"
                    ...
                    `);
                    return html`
                        <div className="DataTable5-header">
                            ${header}
                        </div>
                    `;
                })();
        ...
        renderFilterHeader() {
            ...
            if (this.props.columns) {
                return (() => {
                    ...
                    `);
                    return html`
                        <div className="DataTable5-filter-header">${header}</div>
                    `;
                })();
            }
            return (() => {
            ...
            */
                ...
                if (this.props.renderDetail) {
                    return html`
                        <${IconButton} 
                            ...
                            active=${showDetail}
                            onClick=${(shiftPressed) => {this.onRowClick(rowIndex, shiftPressed);}}
                        />
                    `;
                }
                return null;
            ...
            const result = html`
                <div className=${rowClasses.join(' ')} role="row">
                    ${row}
                    <div className="DataTable5-detail">
            ...
            return this.props.columns.map((col, columnIndex) => {
                ...
                const content = (() => {
                    if (col.render) {
                        ...
                        return html`
                            <div className="DataTable5-col-content" title=${domSafeText(values[col.id])}>
                                ${values[col.id]}
                            </div>
                        `;
                    }
                ...
                const isHovered = this.state.columnHovered === columnIndex;
                if (isHovered) {
                    columnClasses.push('DataTable5-col-hovered');
                }
        ...
        onRowClick(clickedIndex, shiftPressed) {
            if (!this.props.renderDetail) {
                return;
            }
        ...
        renderTable() {
            ...
            if (this.props.flex) {
                classes.push('DataTable5-flex');
            }
            if (this.props.bordered) {
                classes.push('DataTable5-bordered');
            }
            return html`
                    ...
                    const showForColumnFilter = filters.every((filter) => {
                        ...
                        const value = (() => {
                            if (column.filterValue) {
                                return column.filterValue(row.values[columnId]);
                            }
        ...
        renderColumnToolbar() {
            ...
            `;
            return html`
                <div className="DataTable5-toolbar">
                ${toolbar}
                </div>
            `;
        }
        ...
        // Render all
        render() {
            const classes= ['DataTable5-wrapper'];
            if (this.props.flex) {
                classes.push('DataTable5-flex');
            }
            // ${this.renderToolbar()}
