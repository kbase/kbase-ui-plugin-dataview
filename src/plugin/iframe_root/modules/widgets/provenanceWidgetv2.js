...
define([
    ...
    function widget(config) {
        ...
        function renderLayout() {
            return div([
                div([
                    'This is a visualization of the relationships between this piece of data and other data in KBase.  Click objects to show additional information (shown below the graph). Double click on an object expand graph.',
                    br(),
                    br()
                ]),
                div({
                    id: 'objgraphview2',
                    style: { overflow: 'auto', height: '' + (svgHeight * 3) / 4 + 'px', resize: 'vertical' }
                }),
        ...
        function addNodeColorKey() {
            if (needColorKey) {
                ...
                Object.keys(types).map(function (type) {
                    ...
                    var colorName = $('<td/>', { text: types[type].name }).css('vertical-align', 'top');
            ...
            if (d.isFake) {
                ...
                try {
                    ...
                    var info = objdata.info,
                        ...
                        text = '<center><table cellpadding="2" cellspacing="0" class="table table-bordered"><tr><td>';
                    text +=
                        '<h4>Data Object Details</h4><table cellpadding="2" cellspacing="0" border="0" class="table table-bordered table-striped">';
                    text +=
                        ...
                        '</a>)</td></tr>';
                    text +=
                        '<tr><td><b>Type</b></td><td><a href="/#spec/type/' +
                        info[2] +
                        '" target="_parent">' +
                        info[2] +
                        '</a></td></tr>';
                    ...
                    text +=
                        '<tr><td><b>Saved by</b></td><td><a href="/#people/' +
                        info[5] +
                        '" target="_blank">' +
                        info[5] +
                        '</td></tr>';
                    ...
                    if (objdata.copied) {
                        text += getTableRow(
                            'Copied from',
                            '<a href="/#dataview/' + objdata.copied + '" target="_blank">' + objdata.copied + '</a>'
                        );
                    }
                    if (objdata.provenance.length > 0) {
                        ...
                        text +=
                            '<tr><td></td><td><b><span style="color:red">No provenance data set.</span></b></td></tr>';
                    }
                    text += '</table>';
                    text += '</td></tr></table>';
                    $container.find('#objdetailsdiv').html(text);
            ...
            if ('method_params' in provenanceAction) {
                rows.push(
                    getTableRow(
                        ...
                        JSON.stringify(scrub(provenanceAction['method_params']), null, '  ')
                    )
                );
            }
            ...
            if ('intermediate_incoming' in provenanceAction) {
                if (provenanceAction['intermediate_incoming'].length > 0)
                    rows.push(
                        getTableRow(
                            prefix + 'Action Input',
                            JSON.stringify(provenanceAction['intermediate_incoming'], null, '  ')
                        )
                    );
            }
                ...
                if (provenanceAction['intermediate_outgoing'].length > 0)
                    rows.push(
                        getTableRow(
                            ...
                            JSON.stringify(provenanceAction['intermediate_outgoing'], null, '  ')
                        )
                    );
            }
                ...
                if (provenanceAction['external_data'].length > 0) {
                    rows.push(
                        getTableRow(
                            ...
                            formatProvenanceExternalData(provenanceAction['external_data']),
                            null,
                            '  '
                        )
                    );
        ...
        function getReferencingObjects(objectIdentity) {
            ...
            return workspace
                .list_referencing_objects([objectIdentity])
                .then(function (refData) {
                    var objectIds = getObjectIds(refData[0]);
                    if (refData[0].length === 0) {
                        return null;
                    }
                    return workspace
                        .get_objects2({
            ...
            for (var i = 0; i < provData.data.length; i++) {
                ...
                for (var j = 0; j < data.refs.length; j++) {
                    if (objectIdentity.ref === data.refs[j]) {
                        var isDep = true;
                        var objIdGen = objectIdentity.ref
                            .split('/')
                            .slice(0, 2)
                            .join('/');
                    ...
                    if (provenance.resolved_ws_objects) {
                        provenance.resolved_ws_objects.forEach(function (resolvedObjectRef) {
                            if (!(resolvedObjectRef in uniqueRefs)) {
                                ...
                                if (nodePaths[objectIdentity.ref] === undefined)
                                    nodePaths[objectIdentity.ref] = objectIdentity.ref;
        ...
        function buildDataAndRender(objref) {
            ...
            workspace
                ...
                .then(function (objectIdentity) {
                    return Promise.all([getObjectProvenance(objectIdentity), getReferencingObjects(objectIdentity)]);
                })
                .then(function () {
                    finishUpAndRender();
                })
        ...
        function renderForceTree(nodesData, linksData) {
            ...
            var width = svgWidth,
                ...
                nodes = nodesData,
                links = linksData,
                t = d3.transition().duration(1750); // d3 selections
            ...
            force.on('tick', tick);
            svg = d3
                .select($container.find('#objgraphview2')[0])
                .append('svg')
                .attr('width', width)
                .attr('height', height)
                .attr('class', 'prov');
            ...
            function update() {
                ...
                background.attr('height', svgHeight).attr('width', svgWidth);
                force.nodes(nodes).links(links);
                var n = svg.selectAll('.node').data(nodes, function (d) {
                    return d.objId;
                });
                var l = svg.selectAll('.link').data(links, function (d) {
            ...
            function enterNodes(n) {
                ...
                g.append('title').html(function (d) {
                    //0:obj_id, 1:obj_name, 2:type ,3:timestamp, 4:version, 5:username saved_by, 6:ws_id, 7:ws_name, 8 chsum, 9 size, 10:usermeta
                ...
                text.append('tspan')
                    ...
                    .attr('x', function (d) {
                        return -rectWidth / 2 + (d.isFunction ? 40 : 5);
                    })
                ...
                text.append('tspan')
                    .attr('dy', 15)
                    .attr('x', function (d) {
                        return -rectWidth / 2 + (d.isFunction ? 12 : 5);
                    })
                    .attr('y', -rectHeight / 2 + 15)
                    .text(function (d) {
                        if (d.isFunction) {
            ...
            function tick() {
                ...
                link.attr('x1', function (d) {
                    ...
                    .attr('y1', function (d) {
                        return d.source.y + rectHeight / 2;
                    })
                    .attr('x2', function (d) {
                        return d.target.x;
                    })
                    .attr('y2', function (d) {
                        return d.target.y - rectHeight / 2;
                    })
                    .attr('display', function (d) {
                        return d.toggle === false ? 'none' : 'initial';
                    });
            ...
            function dblClick(node) {
                ...
                if (node.isPresent || node.isFunction) {
                    ...
                    node.expanded = true;
                    return Promise.all([provHelper(nodeId, true, [node.data]), getReferencingObjects(nodeId)]).then(
                        function () {
                            nodes = dagreNodes();
                            update();
                            node.isPresent = true;
            ...
            function toggleNode(node) {
                ...
                while (queue.length > 0) {
                    ...
                    if (
                        (condition === false && !hasLinkDep(link.target)) ||
                        (condition === true && condition !== link.target.toggle)
                    ) {
