define([
    ...
    'd3_sankey'
], (Promise, $, d3, html, dom, Workspace) => {
    function widget(config) {
        const runtime = config.runtime;
        const workspace = new Workspace(runtime.getConfig('services.workspace.url'), {
        ...
        function renderLayout() {
            return div([
                div([
                    'This is a visualization of the relationships between this piece of data and other data in KBase.  Mouse over objects to show additional information (shown below the graph). Double click on an object to select and recenter the graph on that object in a new window.',
                    br(),
                    br()
                ]),
                ...
                const content = table(
                    {
                        ...
                        border: '0',
                        width: '100%',
                        style: {border: '1px silver solid; padding: 4px;'}
                    },
                    [
                        tr([
                            td({valign: 'top'}, [
                                    ...
                                    Object.keys(types)
                                        .map((type) => {
                                            ...
                                            return tr([
                                                ...
                                                td({valign: 'middle'}, [types[type].name])
                                            ]);
                                        })
                                        .filter((x) => {
                                            if (x === undefined) {
            ...
            if (d.isFake) {
                ...
                workspace
                    ...
                    .then((objdata) => {
                        ...
                        let found = false,
                            text =
                                '<center><table cellpadding="2" cellspacing="0" class="table table-bordered"><tr><td>';
                        text +=
                        ...
                        let metadata =
                            '<tr><td><b>Meta data</b></td><td><div style="width:250px;word-wrap: break-word;">';
                        ...
                        if (objdata.length > 0) {
                            ...
                            if (objdata[0]['provenance'].length > 0) {
                                let prefix = '';
                                for (let k = 0; k < objdata[0]['provenance'].length; k++) {
        ...
        function renderSankeyStyleGraph() {
            ...
            const svg = d3.select($container.find('#objgraphview')[0]).append('svg');
        ...
        function getReferencingObjects(objIdentities) {
            return workspace.list_referencing_objects(objIdentities).then((refData) => {
        ...
        function getObjectProvenance(objIdentities) {
            return workspace.get_object_provenance(objIdentities).then((objdata) => {
                const uniqueRefs = {},
        ...
        function buildDataAndRender(objref) {
            ...
            workspace
                ...
                .then((objIdentities) => {
                    ...
                    return Promise.all([getReferencingObjects(objIdentities), getObjectProvenance(objIdentities)]);
                })
                .spread((ignore, refData) => {
