...
define(['jquery', 'widgets/communities/jquery.svg'], ($) => {
    ...
    const standaloneGraph = {
        ...
        drawImage(svg, cmax) {
            ...
            svg.graph.xAxis
                .title(this.settings.x_title, this.settings.x_title_color)
            ...
            svg.graph.yAxis
                ...
                .ticks(
                    sy.max / this.settings.y_labeled_tick_interval,
                    sy.max / this.settings.y_tick_interval,
                    null,
                    null,
            ...
            if (this.settings.hasY2) {
                svg.graph.y2Axis
                    ...
                    .ticks(
                        parseInt(y2max / this.settings.y2_labeled_tick_interval, 10),
                        parseInt(y2max / this.settings.y2_tick_interval, 10),
                        null,
                        null,
                        this.settings.y_scale
                    )
            ...
            for (let i = 0; i < this.settings.data.length; i += 1) {
                svg.graph
                    .noDraw()
                    .series(i)
                    .format(this.settings.data[i].fill || fills[i])
                    .end();
            }
            svg.graph
                .noDraw()
                .area(this.settings.chartArea || chartAreas[chartLegend])
            ...
            for (let i = 0; i < data.length; i += 1) {
                ...
                if (data[i].data.length % 2 === 1) {
                    ...
                    if (medup % 2 === 1) {
                        ...
                        fivenumbers[i]['upper'] =
                            (data[i].data[medup + medup / 2 - 1] + data[i].data[medup + medup / 2]) / 2;
                    }
                }
            }
