    ...
    $.KBWidget({
        ...
        render: function () {
            ...
            self.tooltip = d3
                .select('body')
                .append('div')
                .classed('genome-comparison-tooltip', true);
            ...
            var cmpIsLoaded = function () {
                ...
                kbws.get_object_subset(
                    [
                        { ref: self.cmp.genome1ref, included: ['scientific_name'] },
                    ...
                    function (data) {
                        ...
                        for (var i in self.cmp.data1) {
                            if (self.cmp.data1[i].length > 0) count1hits++;
                        }
                        var count2hits = 0;
                        for (var i in self.cmp.data2) {
                            if (self.cmp.data2[i].length > 0) count2hits++;
                        }
                        ...
                        table.append(
                            createTableRow(
                                ...
                                '<a href="/#dataview/' +
                                    ...
                                    '</a>' +
                                    ' (' +
                                    self.cmp.proteome1names.length +
                                    ' genes, ' +
                                    count1hits +
                                    ' have hits)'
                            )
                        ...
                        table.append(
                            createTableRow(
                                ...
                                '<a href="/#dataview/' +
                                    ...
                                    '</a>' +
                                    ' (' +
                                    self.cmp.proteome2names.length +
                                    ' genes, ' +
                                    count2hits +
                                    ' have hits)'
                            )
                        ...
                        table.append(
                            '<tr><td>' +
                                ...
                                '<div id="' +
                                self.pref +
                                'rect" style="position:absolute; z-index: 2; border: 1px; border-style: solid; border-color: red; background-color: transparent; display:none; pointer-events:none;"/>' +
                                '</div>' +
                                '</td>' +
                                '<td width="300"' +
                                sd +
                        ...
                        var zoom = function (mult) {
                            ...
                            if (
                                (self.size * 100) / (self.scale * mult) >
                                1.1 * Math.max(self.cmp.proteome1names.length, self.cmp.proteome2names.length)
                            ) {
                                return;
                            }
                            self.scale *= mult;
                        ...
                        $('#' + self.pref + 'img')
                            ...
                            .mousemove(function (e) {
                                ...
                                if (Number(hit.bestDist) >= 0) {
                                    var msg =
                                        'X-axis: ' +
                                        self.cmp.proteome1names[Number(hit.bestI)] +
                                        ', Y-axis: ' +
                                        self.cmp.proteome2names[Number(hit.bestJ)] +
        ...
        refreshImage: function () {
            ...
            if (maxI > self.cmp.proteome1names.length) maxI = self.cmp.proteome1names.length;
            var maxJ = self.imgJ + (self.size * 100) / self.scale;
            if (maxJ > self.cmp.proteome2names.length) maxJ = self.cmp.proteome2names.length;
            self.imgI = maxI - (self.size * 100) / self.scale;
            self.imgJ = maxJ - (self.size * 100) / self.scale;
            if (self.imgI < 0) self.imgI = 0;
            if (self.imgJ < 0) self.imgJ = 0;
            ...
            var img =
                ...
                self.scale +
                '&token=' +
                encodeURIComponent(self.authToken());
        ...
        refreshGenes: function () {
            ...
            var arrowI = '&#8595;';
            var arrowJ = '&#8595;';
            if (self.dirI < 0) arrowI = '&#8593;';
            if (self.dirJ < 0) arrowJ = '&#8593;';
            var sr = ' style="border: 0px; margin: 0px; padding: 0px;"';
            var sd = ' style="border: 0px; margin: 0px; padding: 1px;"';
            var sb = ' style="width: 27px;"';
            tbl.append(
                '<tr' +
                    ...
                    '<td rowspan="' +
                    (self.geneRows + 2) +
                    '" width="10" style="border: 0px; margin: 0px; padding: 0px; text-align: center; vertical-align: middle;"><button id="' +
                    self.pref +
                    'btn-dirI"' +
                    sb +
                    '>' +
                    arrowI +
                    '</button></td>' +
                    '<td style="border: 0px; margin: 0px; padding: 0px; text-align: center; vertical-align: middle;"><button id="' +
                    self.pref +
                    'btn-i-up"' +
                    sb +
                    '>&#8593;</button></td>' +
            ...
            for (var rowPos = 0; rowPos < self.geneRows; rowPos++) {
                ...
                var labelI = '-';
                var labelJ = '-';
                if (i >= 0 && i < self.cmp.proteome1names.length) labelI = self.cmp.proteome1names[i];
                if (j >= 0 && j < self.cmp.proteome2names.length) labelJ = self.cmp.proteome2names[j];
                if (rowPos === half) {
                    labelI = '<font color="red">' + labelI + '</font>';
                    labelJ = '<font color="red">' + labelJ + '</font>';
                }
                var tdSt =
                    ' style="border: 0px; margin: 0px; padding: 0px; font-size: 12px; height: ' +
                    self.geneRowH +
                    'px; text-align: center; vertical-align: middle;"';
                var tds =
                    '<td ' +
                    tdSt +
                    '>' +
                ...
                for (var tuplePos in self.cmp.data1[i]) {
                    ...
                    if (hitPos >= 0 && hitPos < self.geneRows) {
                        ...
                        var dash = '';
                        if (tuple[2] < 100) dash = ' stroke-dasharray="5, 5"';
                        svgLines +=
                            '<line x1="0" y1="' +
                            y1 +
                            '" x2="30" y2="' +
                        ...
                        svgLineEnds.push({
                            ...
                            x2: 30,
                            y2: y2,
                            gene1: self.cmp.proteome1names[i],
                            gene2: self.cmp.proteome2names[hitJ],
                            bit_score: tuple[1],
                            percent_of_bbh: tuple[2]
                        });
                    }
                }
            }
            ...
            svgTd
                ...
                .mousemove(function (e) {
                    ...
                    for (var n in svgLineEnds) {
                        ...
                        var dist =
                            Math.abs((l.y2 - l.y1) * x - (l.x2 - l.x1) * y + l.x2 * l.y1 - l.y2 * l.x1) /
                    ...
                    if (minDist && minDist <= 2) {
                        var msg =
                            ...
                            'Bit-score: ' +
                            bestLine.bit_score +
                            '<br>Percent of related BBH bit-score: ' +
                            bestLine.percent_of_bbh +
                            '%';
                        tip.html(msg);
                        /*tip.css({
        ...
        getState: function () {
            ...
            if (self.scale === null)
                self.scale =
                    (self.size * 100) / Math.max(self.cmp.proteome1names.length, self.cmp.proteome2names.length);
            var state = {
                imgI: self.imgI,
                imgJ: self.imgJ,
                scale: self.scale,
